...
--- and for production
--
local network_utils = require("vim-insight.reporter.network-utils")
local bussin = require("vim-insight.bus")
...
function NetworkReporter:enable()
    ...
    client:connect(self.opts.uri, self.opts.port, function(err)
        ...
        if err ~= nil then
            self.state = "error"
            error("vim-insight failed to connect to the APM server: " .. err)
        else

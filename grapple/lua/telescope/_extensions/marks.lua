...
local grapple_mark = require("grapple.mark")
...
local generate_new_finder = function()
    return finders.new_table({
        results = prepare_results(grapple.get_mark_config().marks),
...
local delete_grapple_mark = function(prompt_bufnr)
    ...
    grapple_mark.rm_file(selection.filename)
    ...
    for _, current_selection in ipairs(selections) do
        grapple_mark.rm_file(current_selection.filename)
    end
...
local move_mark_up = function(prompt_bufnr)
    local selection = action_state.get_selected_entry()
    local length = grapple_mark.get_length()

    if selection.index == length then
        return
    end

    local mark_list = grapple.get_mark_config().marks
...
return function(opts)
    ...
    pickers.new(opts, {
        ...
        attach_mappings = function(_, map)
            map("i", "<c-d>", delete_grapple_mark)
            map("n", "<c-d>", delete_grapple_mark)

...
local user_config = string.format("%s/grapple.json", config_path)
local cache_config = string.format("%s/grapple.json", data_path)

local M = {}

local khulna_soft_grapple = vim.api.nvim_create_augroup(
    "KHULNA_SOFT_GRAPPLE",
    { clear = true }
)
...
vim.api.nvim_create_autocmd({ "BufLeave", "VimLeave" }, {
    callback = function()
        require("grapple.mark").store_offset()
    end,
    group = khulna_soft_grapple,
})
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
        -- Open grapple file choice in useful ways
        --
...
--]]
GrappleConfig = GrappleConfig or {}
...
function M.save()
    ...
    Path:new(cache_config):write(vim.fn.json_encode(GrappleConfig), "w")
end
...
function M.setup(config)
    ...
    if complete_config.tabline then
        require("grapple.tabline").setup(complete_config)
    end

    GrappleConfig = complete_config

    log.debug("setup(): Complete config", GrappleConfig)
    log.trace("setup(): log_key", Dev.get_log_key())
end

function M.get_global_settings()
    log.trace("get_global_settings()")
    return GrappleConfig.global_settings
end
...
function M.refresh_projects_b4update()
    ...
    local current_p_config = {
        projects = {
            [cwd] = ensure_correct_config(GrappleConfig).projects[cwd],
        },
    }

    -- erase all projects from global config, will be loaded back from disk
    GrappleConfig.projects = nil
    ...
    -- don't override non-project config in GrappleConfig later
    ...
    GrappleConfig = complete_config
    log.debug("refresh_projects_b4update(): Complete config", GrappleConfig)
    log.trace("refresh_projects_b4update(): log_key", Dev.get_log_key())
end

function M.get_term_config()
    log.trace("get_term_config()")
    return ensure_correct_config(GrappleConfig).projects[utils.project_key()].term
end

function M.get_mark_config()
    log.trace("get_mark_config()")
    return ensure_correct_config(GrappleConfig).projects[mark_config_key()].mark
end

function M.get_menu_config()
    log.trace("get_menu_config()")
    return GrappleConfig.menu or {}
end

-- should only be called for debug purposes
function M.print_config()
    print(vim.inspect(GrappleConfig))
end

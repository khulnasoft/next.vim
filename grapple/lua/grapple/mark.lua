...
local function emit_changed()
    ...
    local global_settings = grapple.get_global_settings()
...
local function filter_filetype()
    ...
    local excluded_filetypes = grapple.get_global_settings().excluded_filetypes

    if current_filetype == "grapple" then
        log.error("filter_filetype(): You can't add grapple to the grapple")
        error("You can't add grapple to the grapple")
        return
    end
...
function M.get_index_of(item, marks)
    ...
    if item == nil then
        ...
        error(
            "You have provided a nil value to Grapple, please provide a string rep of the file or the file idx."
        )
        return
    end
    ...
    if type(item) == "string" then
        ...
        if marks == nil then
            marks = grapple.get_mark_config().marks
        end
    ...
    -- TODO move this to a "grapple_" prefix or global config?
...
function M.add_file(file_name_or_buf_id)
    ...
    grapple.get_mark_config().marks[found_idx] = create_mark(buf_name)
...
function M.remove_empty_tail(_emit_on_changed)
    ...
    local config = grapple.get_mark_config()
    local found = false
...
function M.store_offset()
    ...
    local ok, res = pcall(function()
        local marks = grapple.get_mark_config().marks
...
function M.rm_file(file_name_or_buf_id)
    ...
    grapple.get_mark_config().marks[idx] = create_mark("")
    M.remove_empty_tail(false)
    emit_changed()
end

function M.clear_all()
    grapple.get_mark_config().marks = {}
...
function M.get_marked_file(idxOrName)
    ...
    return grapple.get_mark_config().marks[idxOrName]
end
...
function M.get_marked_file_name(idx, marks)
    ...
    if marks ~= nil then
        ...
        mark = grapple.get_mark_config().marks[idx]
    end

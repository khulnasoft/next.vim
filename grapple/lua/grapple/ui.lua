...
local Marked = require("grapple.mark")
...
local function close_menu(force_save)
    ...
    if global_config.save_on_toggle or force_save then
        require("grapple.ui").on_menu_save()
    end

    vim.api.nvim_win_close(Grapple_win_id, true)
...
local function create_window()
    ...
    local Grapple_win_id, win = popup.create(bufnr, {
        title = "Grapple",
    ...
    return {
        bufnr = bufnr,
        win_id = Grapple_win_id,
    }
end

local function get_menu_items()
    log.trace("_get_menu_items()")
    local lines = vim.api.nvim_buf_get_lines(Grapple_bufh, 0, -1, true)
    local indices = {}
...
function M.toggle_quick_menu()
    ...
    if Grapple_win_id ~= nil and vim.api.nvim_win_is_valid(Grapple_win_id) then
        close_menu()
        return
    end
    ...
    Grapple_win_id = win_info.win_id
    Grapple_bufh = win_info.bufnr
    ...
    vim.api.nvim_win_set_option(Grapple_win_id, "number", true)
    vim.api.nvim_buf_set_name(Grapple_bufh, "grapple-menu")
    vim.api.nvim_buf_set_lines(Grapple_bufh, 0, #contents, false, contents)
    vim.api.nvim_buf_set_option(Grapple_bufh, "filetype", "grapple")
    vim.api.nvim_buf_set_option(Grapple_bufh, "buftype", "acwrite")
    vim.api.nvim_buf_set_option(Grapple_bufh, "bufhidden", "delete")
    vim.api.nvim_buf_set_keymap(
        Grapple_bufh,
        "n",
        "q",
        "<Cmd>lua require('grapple.ui').toggle_quick_menu()<CR>",
        { silent = true }
    )
    vim.api.nvim_buf_set_keymap(
        Grapple_bufh,
    ...
    vim.api.nvim_buf_set_keymap(
        ...
        "n",
        "<CR>",
        "<Cmd>lua require('grapple.ui').select_menu_item()<CR>",
        {}
    )
    vim.cmd(
        string.format(
            "autocmd BufWriteCmd <buffer=%s> lua require('grapple.ui').on_menu_save()",
            Grapple_bufh
        )
    )
    if global_config.save_on_change then
        vim.cmd(
            string.format(
                "autocmd TextChanged,TextChangedI <buffer=%s> lua require('grapple.ui').on_menu_save()",
                Grapple_bufh
            )
        )
    end
    ...
    vim.cmd(
        "autocmd BufLeave <buffer> ++nested ++once silent lua require('grapple.ui').toggle_quick_menu()"
    )
end

...
if global_config.tmux_autoclose_windows then
    ...
    vim.api.nvim_create_autocmd("VimLeave", {
        callback = function()
            require("grapple.tmux").clear_all()
        end,
...
function M.sendCommand(idx, cmd, ...)
    ...
    if type(cmd) == "number" then
        cmd = grapple.get_term_config().cmds[cmd]
    end
...
function M.get_length()
    ...
    return table.maxn(grapple.get_term_config().cmds)
end
...
function M.emit_changed()
    ...
    if grapple.get_global_settings().save_on_change then
        grapple.save()
    end
end
...
function M.add_cmd(cmd)
    ...
    grapple.get_term_config().cmds[found_idx] = cmd
    M.emit_changed()
end
...
function M.rm_cmd(idx)
    ...
    table.remove(grapple.get_term_config().cmds, idx)
    M.emit_changed()
end

function M.set_cmd_list(new_list)
    log.trace("set_cmd_list(): New list:", new_list)
    for k in pairs(grapple.get_term_config().cmds) do
        grapple.get_term_config().cmds[k] = nil
    end
    for k, v in pairs(new_list) do
        grapple.get_term_config().cmds[k] = v
    end
    M.emit_changed()
end

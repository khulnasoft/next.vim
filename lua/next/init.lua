...
local user_config = string.format("%s/next.json", config_path)
local cache_config = string.format("%s/next.json", data_path)

local M = {}

local khulnasoft_next =
    vim.api.nvim_create_augroup("KHULNASOFT_NEXT", { clear = true })

vim.api.nvim_create_autocmd({ "BufLeave", "VimLeave" }, {
    callback = function()
        require("next.mark").store_offset()
    end,
    group = khulnasoft_next,
})
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
        -- Open next file choice in useful ways
        --
...
function M.save()
    ...
    Path:new(cache_config):write(vim.fn.json_encode(NextConfig), "w")
end
...
function M.setup(config)
    ...
    if complete_config.tabline then
        require("next.tabline").setup(complete_config)
    end

    NextConfig = complete_config

    log.debug("setup(): Complete config", NextConfig)
    log.trace("setup(): log_key", Dev.get_log_key())
end

function M.get_global_settings()
    log.trace("get_global_settings()")
    return NextConfig.global_settings
end
...
function M.refresh_projects_b4update()
    ...
    local current_p_config = {
        projects = {
            [cwd] = ensure_correct_config(NextConfig).projects[cwd],
        },
    }

    -- erase all projects from global config, will be loaded back from disk
    NextConfig.projects = nil
    ...
    -- don't override non-project config in NextConfig later

...
if global_config.tmux_autoclose_windows then
    local next_tmux_group =
        vim.api.nvim_create_augroup("NEXT_TMUX", { clear = true })

    vim.api.nvim_create_autocmd("VimLeave", {
        callback = function()
            require("next.tmux").clear_all()
        end,
        group = next_tmux_group,
    })
end
...
function M.sendCommand(idx, cmd, ...)
    ...
    if type(cmd) == "number" then
        cmd = next.get_term_config().cmds[cmd]
    end
...
function M.get_length()
    ...
    return table.maxn(next.get_term_config().cmds)
end
...
function M.emit_changed()
    ...
    if next.get_global_settings().save_on_change then
        next.save()
    end
end
...
function M.add_cmd(cmd)
    ...
    next.get_term_config().cmds[found_idx] = cmd
    M.emit_changed()
end
...
function M.rm_cmd(idx)
    ...
    table.remove(next.get_term_config().cmds, idx)
    M.emit_changed()
end

function M.set_cmd_list(new_list)
    log.trace("set_cmd_list(): New list:", new_list)
    for k in pairs(next.get_term_config().cmds) do
        next.get_term_config().cmds[k] = nil
    end
    for k, v in pairs(new_list) do
        next.get_term_config().cmds[k] = v
    end
    M.emit_changed()
end

...
local Marked = require("next.mark")
...
local function close_menu(force_save)
    ...
    if global_config.save_on_toggle or force_save then
        require("next.ui").on_menu_save()
    end

    vim.api.nvim_win_close(Next_win_id, true)
...
local function create_window()
    ...
    local Next_win_id, win = popup.create(bufnr, {
        title = "Next",
...
local function get_menu_items()
    ...
    local lines = vim.api.nvim_buf_get_lines(Next_bufh, 0, -1, true)
    local indices = {}
...
function M.toggle_quick_menu()
    ...
    if Next_win_id ~= nil and vim.api.nvim_win_is_valid(Next_win_id) then
        close_menu()
        return
    end
    ...
    Next_win_id = win_info.win_id
    Next_bufh = win_info.bufnr
    ...
    vim.api.nvim_win_set_option(Next_win_id, "number", true)
    vim.api.nvim_buf_set_name(Next_bufh, "next-menu")
    vim.api.nvim_buf_set_lines(Next_bufh, 0, #contents, false, contents)
    vim.api.nvim_buf_set_option(Next_bufh, "filetype", "next")
    vim.api.nvim_buf_set_option(Next_bufh, "buftype", "acwrite")
    vim.api.nvim_buf_set_option(Next_bufh, "bufhidden", "delete")
    vim.api.nvim_buf_set_keymap(
        Next_bufh,
        "n",
        "q",
        "<Cmd>lua require('next.ui').toggle_quick_menu()<CR>",
        { silent = true }
    )
    vim.api.nvim_buf_set_keymap(
        Next_bufh,
    ...
    vim.api.nvim_buf_set_keymap(
        ...
        "n",
        "<CR>",
        "<Cmd>lua require('next.ui').select_menu_item()<CR>",
        {}
    )
    vim.cmd(
        string.format(
            "autocmd BufWriteCmd <buffer=%s> lua require('next.ui').on_menu_save()",
            Next_bufh
        )
    )
    if global_config.save_on_change then
        vim.cmd(
            string.format(
                "autocmd TextChanged,TextChangedI <buffer=%s> lua require('next.ui').on_menu_save()",
                Next_bufh
            )
        )
    end
    ...
    vim.cmd(
        "autocmd BufLeave <buffer> ++nested ++once silent lua require('next.ui').toggle_quick_menu()"
    )
end

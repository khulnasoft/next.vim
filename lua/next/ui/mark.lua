...
local function emit_changed()
    ...
    local global_settings = next.get_global_settings()
...
local function filter_empty_string(list)
    ...
    for idx = 1, #list do
        if list[idx] ~= "" then
            table.insert(result, list[idx].filename)
        end
    end
...
local function filter_filetype()
    ...
    local excluded_filetypes = next.get_global_settings().excluded_filetypes

    if current_filetype == "next" then
        log.error("filter_filetype(): You can't add next to the next")
        error("You can't add next to the next")
        return
    end
...
function M.get_index_of(item, marks)
    ...
    if item == nil then
        ...
        error(
            "You have provided a nil value to Next, please provide a string rep of the file or the file idx."
        )
        return
    end
    ...
    if type(item) == "string" then
        ...
        if marks == nil then
            marks = next.get_mark_config().marks
        end
    ...
    -- TODO move this to a "next_" prefix or global config?
...
function M.add_file(file_name_or_buf_id)
    ...
    next.get_mark_config().marks[found_idx] = create_mark(buf_name)
...
function M.remove_empty_tail(_emit_on_changed)
    ...
    local config = next.get_mark_config()
    local found = false
...
function M.store_offset()
    ...
    local ok, res = pcall(function()
        local marks = next.get_mark_config().marks
...
function M.rm_file(file_name_or_buf_id)
    ...
    next.get_mark_config().marks[idx] = create_mark("")
    M.remove_empty_tail(false)
    emit_changed()
end

function M.clear_all()
    next.get_mark_config().marks = {}
...
function M.get_marked_file(idxOrName)
    ...
    return next.get_mark_config().marks[idxOrName]
end
...
function M.get_marked_file_name(idx, marks)
    ...
    if marks ~= nil then
        ...
        mark = next.get_mark_config().marks[idx]
    end
